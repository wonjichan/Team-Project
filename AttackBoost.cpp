...
AttackBoost::AttackBoost(string m, int quantity, int price)
    : Item(m, quantity, price), attackIncrease(10) {
}

// 사용 시: 캐릭터의 공격력이 10 증가
// → 전투 종료 시 clearTempBoost()로 초기화해야 함
void AttackBoost::use(Character& player)
{
    cout << player.getName() << "의 공격력이 10 증가했습니다!" << "\n" << endl;
