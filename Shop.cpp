...
void Shop::resetStock()
{
    ...
    cout << "【Health Potion: 10 Gold 】 【Attack Potion: 15 Gold 】\n";

    for (size_t i = 0; i < Shop_Items.size(); i++)
    {
        cout << "[" << i + 1 << "] " << Shop_Items[i]->getName()
            << ", " << Shop_Items[i]->getQuantity() << endl;
    }
}
...
void Shop::buyItem(int index, Character* player)
{
    ...
    if (index < 0 || index >= static_cast<int>(Shop_Items.size()))
    {
        cout << "잘못된 선택입니다.\n";
        return;
    }
...
void Shop::sellItem(int index, Character* player)
{
    ...
    if (items.empty() || items[index]->getQuantity() <= 0)
    {
        cout << "판매할 아이템이 없습니다.\n";
        return;
    }
    ...
    cout << items[index]->getName() << "을(를) " << count
        << "개 판매했습니다. +" << sellPrice << " Gold\n";
}
...
void Shop::show_ShopItems() const
{
    ...
    for (size_t i = 0; i < Shop_Items.size(); i++)
    {
        cout << i + 1 << ": " << Shop_Items[i]->getName() // 아이템 이름 출력
            << " (x" << Shop_Items[i]->getQuantity() << ")\n"; // 아이템 수량 출력
    }
}
