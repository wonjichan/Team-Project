...
public:
    // 생성자: 이름과 수량, 가격을 받아 초기화
    Item(const std::string& name, int quantity, int price);

    // 가상 소멸자: 상속받은 클래스에서 안전한 삭제 가능
    ...
    void reduceQuantity(int amount);// 아이템 수량 감소 (음수 방지)

    // 순수 가상 함수 → 상속받은 클래스에서 반드시 구현
