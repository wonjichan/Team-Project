...
Inventory::~Inventory() {
    ...
    {   // items 벡터에 들어있는 아이템 포인터들을 하나씩 꺼냄
        delete item;            // 메모리 해제 (new로 만든 객체니까 delete 필요)
    }
}
...
void Inventory::addItem(Item* item) 
{
    ...
    {   // 현재 인벤토리에 들어 있는 아이템들 순회
        ...
        {  // 이름이 같으면 같은 종류의 아이템
            ...
            delete item;   // 새로 추가하려던 아이템은 불필요하므로 메모리 해제
            return;        // 함수 종료
        }
    }
    items.push_back(item);   // 같은 종류가 없으면 새로 추가
}
...
void Inventory::showItems() const 
{
    ...
    {
        std::cout << i << ": " << items[i]->getName()   // 아이템 이름 출력
